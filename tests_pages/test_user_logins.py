...
class TestUserLogins:
    ...
    @pytest.mark.smoke
    @pytest.mark.login
    def test_login_with_valid_credentials_with_pom(self, home_page: HomePage) -> None:
        """Verify successful login with valid credentials using POM."""
        home_page.go_to_home_page()
        home_page.verify_home_page_loads()
        home_page.login_user()
        # Add assertions for successful login (e.g., redirect, dashboard element)

    @pytest.mark.login
    def test_login_with_custom_credentials(self, home_page: HomePage) -> None:
